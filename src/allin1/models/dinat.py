...
# --- API Agnostic Imports ---
# Use an environment variable to switch between APIs. Default to MPS.
NATTEN_API = os.environ.get("NATTEN_API", "mps")

if NATTEN_API == "mps":
    from natten import natten1d, natten2d
else: # cpu/cuda
...
class _NeighborhoodAttentionNd(ABC, nn.Module):
  ...
  def forward(
    ...
    original_dims: Optional[Tuple[int, ...]] = None,
  ) -> Tuple[torch.Tensor]:
    ...
    if NATTEN_API == "mps":
        ...
        # CPU/CUDA Backend Path (Original Logic)
...
class _NeighborhoodAttentionModuleNd(ABC, nn.Module):
  ...
  def forward(
    ...
    self_outputs = self.self(hidden_states, output_attentions, original_dims)
  ...
  def forward(
    ...
    for attention in [self.attention, self.attention2]:
      ...
      if is_2d:
        ...
        if was_padded:
          attention_output = attention_output[:, :original_dims[0], :original_dims[1], :].contiguous()
      else:
        was_padded = pad_values[3] > 0
        if was_padded:
          attention_output = attention_output[:, :original_dims[0], :].contiguous()
